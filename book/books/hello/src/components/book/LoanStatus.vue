  ...
  methods: {
    async toggleGiveStatus(row) {
      try {
        ...
        await axios.put(`http://seqnqc.natappfree.cc/api/lending/${row.id}/`, payload);
    ...
    async fetchLendingData() {
      try {
        const response = await axios.get('http://seqnqc.natappfree.cc/api/lending/'); // 使用axios发起GET请求
